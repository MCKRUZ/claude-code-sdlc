...
- `commands/` — 27 slash commands (`/sdlc`, `/sdlc-setup`, `/sdlc-status`, `/sdlc-next`, `/sdlc-gate`, `/sdlc-enhance`, `/sdlc-coach`, `/sdlc-review`, `/sdlc-intake`, `/sdlc-brief`, `/sdlc-spec`, `/sdlc-handoff`, `/sdlc-phase-report`, `/sdlc-audit`, `/sdlc-feature`, `/sdlc-experience`, `/sdlc-data`, `/sdlc-rules`, `/sdlc-channel`, `/sdlc-evals`, `/sdlc-harness`, `/sdlc-upgrade`, `/sdlc-revise`, `/sdlc-audit-artifacts`, `/sdlc-version`, `/sdlc-refresh`, `/sdlc-retro`)
...
- **One-command hand-off (the Delegate beat's tool)** — `/sdlc-handoff` / `handoff.py` turns branch + frontmatter + code-host assignment + starting the agent into one step. It refuses (repository untouched) unless the spec passes the DoR, the developer is in the roster and isn't also the spec's own checker, and the team is under its WIP limit (spec 0003's cadence-plan limits) unless `--over-limit "<reason>"` is given. Branch creation, the `status`/`developer` frontmatter commit, and the push all happen over plain `git` in a throwaway worktree — the caller's own checkout, dirty or not, is never touched — so the local half still works with no code-host access; only opening the draft PR and assigning/requesting review needs `gh`, and its failure is reported, not fatal. Idempotency is read from the spec's own branch on `origin`, not the caller's local checkout, since the frontmatter change never reaches a checkout still sitting on main. `--open` starts Claude Code on the branch in plan mode; without it, the exact command is printed
...
- **Steering scorecard (outcomes, not activity)** — `scorecard.py` records loop outcomes to `.sdlc/metrics/loop-events.jsonl` and reports the standard's numbers (accepted-as-is, review-wait median, rework/revert, bounce-back, escaped bugs, the DORA four; security-review wait on its own line). Reads "no data" rather than a fabricated zero, and *refuses* to record the forbidden activity metrics (velocity, story points, PR count, LOC). `steering-scorecard.md` is the client-facing artifact. `scorecard.py import --since <date>` builds the same events straight from GitHub's own history via the `gh` CLI (no stored credential — reuses the machine's existing sign-in): merged PRs, review-wait, deployments and closed `incident`-labelled issues, deduplicated on GitHub's own ids so re-running an overlapping window is a no-op. `github_import.py` is the pure mapping layer (`gh`-shaped dict in, event dict out), kept fully unit-testable with no live calls; hand-recording with `record` still works unchanged
...
uv run scripts/scorecard.py import --repo /tmp/test --since 2026-09-01
uv run scripts/handoff.py --repo /tmp/test --spec /tmp/test/specs/0007-name.md --developer @sam-k
